...
CSpaceVoronoiLayer::CSpaceVoronoiLayer(std::vector<RobotColumn> &_columns, double t, int maxDist_squared)
: DynamicVoronoi(maxDist_squared)
{
  ...
  for (unsigned int col=0; col<columns.size(); col++) {
    ...
    int numPoints = c.vertices.size();
    double minY = DBL_MAX;
    numPoints++;
    std::vector<Point> points(numPoints);
    for(unsigned int i=0; i<c.vertices.size(); i++){
      points[i] = Point(c.vertices[i].first, c.vertices[i].second);
    }
    ...
    if (newlyOccupied) {
      for (int nx=first; nx<=last; nx++) {
        if((countMap.preIncrement(nx,ny) == 1) && updateVoronoi){
          setObstacle(nx,ny);
          collisionChanges.push_back(CollisionChange(nx,ny,true));
        }
      }
    } else {
      for (int nx=first; nx<=last; nx++) {
        if((countMap.preDecrement(nx,ny) == 0) && updateVoronoi){
          ...
          collisionChanges.push_back(CollisionChange(nx,ny,false));
        }
      }
    }
  }
